...
template<typename V> using NetPtr = std::shared_ptr<Net<V>>;
...
template<typename V> class Net {
 public:
  ...
  void init(const NetPtr<V>& other = NetPtr<V>(nullptr));
...
template<typename V>
void Net<V>::init(const NetPtr<V>& other) {
  CHECK_GT(cf_.layer_size(), 0);
  for (int id = 0; id < cf_.layer_size(); ++id) {
    auto f = cf_.layer(id);
    auto l = LayerFactory<V>::create(f);
    if (f.has_init() && f.init().type() == ParameterInitConfig::CLONE) {
      for (auto& o : other->layers()) {
        if (o->name() == l->name()) {
          l->model() = o->model();
        }
      }
      CHECK(l->model()) << "didn't find layer " << l->name() << "in *other*";
    }
